...
```{r}
g_c <- g_c %>%
  left_join(pop, by = c("year" = "time"))
rm(pop)
```

## TEST AV THIBIGA
## Oki, nå fikk jeg denne til å kjøre. Dermed er det nå 10 variabler. Den 10. variabelen skal egentlig hete "pop", men den heter "population_total".
```{r}
...
## TEST av THIBIGA
## Fikk denne til å kjøre. Men nå har vi 12 variabler i stedet for 9. Muligens forsvinner de variablene vi ikke trenger når vi rydder opp. 
```{r}
gdp_pc <- read_csv("data/countries-etc-datapoints/ddf--datapoints--gdppercapita_us_inflation_adjusted--by--geo--time.csv",
col_types = cols(time = col_date(format = "%Y")))
```
...
## Her jeg endret navnet på population_total.x. Dersom vi rydder, vil den nok endres seg til *population_total*

```{r}
g_c <- g_c %>% 
  rename("lifeExp" = "life_expectancy_years") %>% 
  rename ("pop" = "population_total.x") %>% 
